...
class DownloadService extends ChangeNotifier {
  ...
  /// Uses app-specific storage first (most reliable for APK on modern Android).
  Future<String> _saveDir() async {
    // 1) App-specific external storage (most reliable, no special permission needed on API 29+)
    try {
  ...
  Future<String?> enqueue({
    ...
    debugPrint('dir: $savedDir');
    debugPrint('isApk: ${name.toLowerCase().endsWith('.apk')}');

    // Always use private/app storage — most reliable for APK on Android 11+
    try {
      ...
      debugPrint('taskId returned: $taskId');
      if (taskId != null) {
        _addItem(taskId, url, name, savedDir);
        return taskId;
      }
      debugPrint('enqueue returned null');
    } catch (e, st) {
